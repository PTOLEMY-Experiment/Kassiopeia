...
unameOut="$(uname -s)"
case "${unameOut}" in
    Linux*)     KASSIOPEIA_CPUS=${KASSIOPEIA_CPUS:-"$(($(nproc)-1))"};;
    Darwin*)    KASSIOPEIA_CPUS=${KASSIOPEIA_CPUS:-"$(($(sysctl -n hw.physicalcpu)-1))"}
esac
...
cmake -DCMAKE_BUILD_TYPE=$KASSIOPEIA_BUILD_TYPE \
        -DCMAKE_INSTALL_PREFIX=$DIR/$KASSIOPEIA_INSTALL_PREFIX \
        -DKASPER_USE_ROOT=ON \
        -DKASPER_USE_VTK=ON \
        -DKASPER_USE_GSL=ON \
